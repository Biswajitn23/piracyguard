...
const Features: React.FC = () => {
  ...
  return (
    <section ref={ref} className="py-20 bg-black" id="features">
      <div className="container mx-auto px-4 md:px-6">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div 
            className={cn(
              "inline-flex items-center px-3 py-1 rounded-full bg-purple-900/50 text-purple-200 text-sm font-medium mb-4 transition-all duration-700",
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10">
          {features.map((feature, index) => (
            <div 
              key={index}
              className={cn(
                "rounded-xl overflow-hidden shadow-lg transition-all duration-700 scale-hover border border-white/5 bg-black/40 backdrop-blur-sm",
                isInView ? "opacity-100" : "opacity-0 translate-y-4",
                feature.delay
              )}
            >
